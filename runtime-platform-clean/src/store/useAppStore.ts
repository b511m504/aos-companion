...
type AppActions = {
  ...
  goSettings(): void
  goRuntimeLab(): void
  goNfcValidation(): void
  goRuntimeValidation(): void
  goPlayHome(): void
  goPlaySession(): void
  goPlayAssign(): void
  goPlayTable(): void
...
export const useAppStore = create<AppState & AppActions>((set, get) => ({
  ...
  goPlayAssign() {
    ...
    if (!list) {
      set({ errorBanner: "Choose a roster first." })
      return
    }
    set({
      screen: "play_assign",
  ...
  goPlayTable() {
    ...
    const list = get().selectedList
    if (!list) {
      set({ errorBanner: "No active roster." })
      return
    }
    set({
      screen: "play_table",
  ...
  applyScanUid(uid: string) {
    ...
    const playAssign = get().screen === "play_assign"
    let nextEntityId: string | null = null
    if (playAssign) {
      const linked = new Set(all.map((a) => a.entityId))
      nextEntityId =
        selectedList.units.find((u) => !linked.has(u.id))?.id ?? null
    }
    set({
      assignments: all,
      ...
      scanFeedback: playAssign
        ? {
            kind: "success",
            uid: built.assignment.tagUid,
            message: `Tag linked · ${built.assignment.displayName}`
          }
        : {
            kind: "success",
            uid: built.assignment.tagUid,
      ...
      awaitingScan: playAssign && nextEntityId != null,
      selectedEntityId: playAssign ? nextEntityId ?? selectedEntityId : selectedEntityId
    })
    if (playAssign) {
      void import("@/play/playSessionStore").then(({ onAssignScanSuccess }) => {
        onAssignScanSuccess(built.assignment.tagUid, built.assignment.displayName)
      })
      if (nextEntityId) {
