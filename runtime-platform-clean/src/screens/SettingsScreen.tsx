...
export function SettingsScreen() {
  ...
  const goRuntimeValidation = useAppStore((s) => s.goRuntimeValidation)
    ...
    <div className="stack wizard-screen">
      <div className="row-between wizard-header">
        ...
        <div className="row" style={{ gap: 8 }}>
          ...
          <button type="button" className="btn btn-ghost" onClick={() => goRuntimeValidation()}>
            Runtime validation
          </button>
      ...
      <div className="panel stack">
        <h2 className="h2">NFC validation lab</h2>
        <p className="muted" style={{ marginTop: -6 }}>
          ReaderMode timeline, capture-first flash, vibration pulse, OEM overlay flag, and a field checklist (stored
          locally). Shown when dev server is running,{" "}
          <span className="mono">VITE_NFC_VALIDATION_HUD=true</span>, or after opening with{" "}
          <span className="mono">?nfcValidate=1</span>.
        </p>
        <div className="row" style={{ flexWrap: "wrap", gap: 10 }}>
          <button type="button" className="btn btn-primary" onClick={() => goNfcValidation()}>
            Open NFC validation
          </button>
        </div>
      </div>
