...
type ScanHandler = (uid: string, meta: { source: "native" | "simulated" }) => void
...
export class NFCManager {
  ...
  private emitScan(uid: string, meta: { source: "native" | "simulated" }) {
    for (const h of this.handlers) h(uid, meta)
  }

  private tryDeliverScan(rawUid: string, meta: { source: "native" | "simulated" }): void {
    ...
    try {
      ...
      const onTag = (ev: Event) => {
        try {
          ...
          const tagInfo = raw.tagInfo as { uid?: string } | undefined
          const uid = tagInfo?.uid
          if (!uid || typeof uid !== "string") {
            this.emitHardwareError({
              at: new Date().toISOString(),
              code: "malformed_payload",
              message: "Native read: missing tagInfo.uid"
            })
            this.schedule(() => {
              if (this.sessionActive) this.transition("scanning")
              else this.transition("idle")
